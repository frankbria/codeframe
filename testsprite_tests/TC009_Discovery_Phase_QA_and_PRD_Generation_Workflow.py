...
async def run_test():
    ...
    try:
        ...
        await page.wait_for_timeout(3000)
        await elem.fill(
            "This project will automate testing of discovery phase interaction and PRD generation."
        )
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator("text=Project Completed Successfully").first).to_be_visible(
                timeout=1000
            )
        except AssertionError:
            raise AssertionError(
                "Test case failed: The discovery phase interaction, question-answer progression, and PRD generation did not complete successfully as expected in the test plan."
            )
