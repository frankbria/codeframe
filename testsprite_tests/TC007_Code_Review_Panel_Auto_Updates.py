...
async def run_test():
    ...
    try:
        ...
        await elem.fill(
            "This project is for testing WebSocket review events and automatic panel updates."
        )
        ...
        try:
            await expect(
                frame.locator("text=Review Panel Successfully Updated").first
            ).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError(
                "Test failed: The review panel did not update automatically upon receiving WebSocket review events as expected in the test plan."
            )
