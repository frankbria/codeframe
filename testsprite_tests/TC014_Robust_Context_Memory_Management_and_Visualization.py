...
async def run_test():
    ...
    try:
        ...
        await page.wait_for_timeout(3000)
        await elem.fill("This project is for testing tiered context memory visualization.")
        ...
        elem = frame.locator("xpath=html/body/main/div/div[2]/form/div/input").nth(0)
        await page.wait_for_timeout(3000)
        await elem.fill("project1")

        frame = context.pages[-1]
        # Input valid description with at least 10 characters
        elem = frame.locator("xpath=html/body/main/div/div[2]/form/div[2]/textarea").nth(0)
        await page.wait_for_timeout(3000)
        await elem.fill("Testing tiered context memory visualization")
        ...
        try:
            await expect(
                frame.locator("text=Tiered Context Memory Visualization Successful").first
            ).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError(
                "Test plan failed: The tiered (HOT/WARM/COLD) context memory visualization did not update automatically with accurate token usage or support filtering and pagination as expected."
            )
