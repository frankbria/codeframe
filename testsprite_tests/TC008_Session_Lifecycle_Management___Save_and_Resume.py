...
async def run_test():
    ...
    try:
        ...
        await page.wait_for_timeout(3000)
        await elem.fill(
            "This project is to test session save and resume functionality with multiple tasks and blockers."
        )
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator("text=Session Save Complete").first).to_be_visible(
                timeout=1000
            )
        except AssertionError:
            raise AssertionError(
                "Test failed: The session state was not saved, persisted, or resumed correctly as per the test plan. Expected session save confirmation message 'Session Save Complete' is missing, indicating failure in session state restoration, progress visualization, or blocker display."
            )
