...
async def run_test():
    ...
    try:
        ...
        await page.wait_for_timeout(3000)
        await elem.fill("This project is for testing the review panel display.")
        ...
        try:
            await expect(
                frame.locator("text=Code Review Passed with Flying Colors").first
            ).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError(
                "Test plan execution failed: ReviewResultsPanel did not display code review scores, status badges, or findings with proper color coding as expected."
            )
