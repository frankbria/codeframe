...
async def run_test():
    ...
    try:
        ...
        await elem.fill("This project tests WebSocket connection and message handling.")
        ...
        try:
            await expect(
                frame.locator("text=WebSocket connection established successfully").first
            ).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError(
                "Test failed: The WebSocket client did not maintain connection with exponential backoff reconnection strategy or failed to process all supported message types including conflict resolution as specified in the test plan."
            )
