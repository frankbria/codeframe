...
async def run_test():
    ...
    try:
        ...
        await elem.fill(
            "This project will test API endpoints for performance and data type validation."
        )
        ...
        try:
            await expect(
                frame.locator("text=API Endpoint Performance Validation Passed").first
            ).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError(
                "Test plan execution failed: API endpoints did not meet performance thresholds or data type validations as required."
            )
