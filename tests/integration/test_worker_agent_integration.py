...
class TestRealApiExecution:
    ...
    async def test_execute_with_haiku_model(self, agent, simple_task):
        ...
        result = await agent.execute_task(simple_task, model_name="claude-haiku-4-5")
...
class TestConcurrentExecution:
    ...
    async def test_multiple_agents_concurrent(self, temp_database, project_id):
        ...
        results = await asyncio.gather(
            agent1.execute_task(task1, model_name="claude-haiku-4-5"),
            agent2.execute_task(task2, model_name="claude-haiku-4-5"),
        )
