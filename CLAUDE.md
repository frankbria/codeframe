...
a day nobody pushed, which is why a red scheduled run opens or updates an issue
via `scheduled-failure-issue.yml` (#1239, #1242) — on PRs touching
`pyproject.toml` or the guards, and as a required gate on `release.yml`. `tests/ci/test_unlocked_resolution_guard_1169.py`
