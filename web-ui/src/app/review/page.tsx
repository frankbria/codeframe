...
export default function ReviewPage() {
  ...
  // Clamp selectedFileIndex when diffFiles shrinks (e.g., after commit)
  useEffect(() => {
    if (diffFiles.length === 0) {
      setSelectedFileIndex(0);
      setSelectedFile(null);
    } else if (selectedFileIndex >= diffFiles.length) {
      const clamped = diffFiles.length - 1;
      setSelectedFileIndex(clamped);
      setSelectedFile(getFilePath(diffFiles[clamped]));
    }
  }, [diffFiles, selectedFileIndex]);

  // Auto-generate commit message on first load only
  const hasAutoGenerated = useRef(false);
  useEffect(() => {
    if (!workspacePath || !diffData || hasAutoGenerated.current) return;
    hasAutoGenerated.current = true;
    reviewApi
      .generateCommitMessage(workspacePath)
      .then((res) => setCommitMessage(res.message))
      .catch(() => {});
  }, [workspacePath, diffData]);
